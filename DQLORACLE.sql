...
GROUP BY 
    CASE WHEN u1.JenisUnit = 'Akademik' THEN u1.NamaUnit
         WHEN u2.JenisUnit = 'Akademik' THEN u2.NamaUnit
         WHEN u3.JenisUnit = 'Akademik' THEN u3.NamaUnit
    END
ORDER BY TotalEmisiKG DESC;

-- ===================================================
-- 3. Top 5 Business Trip (Individu)
-- ===================================================
SELECT * FROM (
    SELECT 
        bt.BTID,
...
-- ===================================================
-- 4. Perbandingan Transportasi & Listrik
-- ===================================================
SELECT 
...
-- ===================================================
-- 5. Top 10 Penyumbang Emisi (Dengan RekapEmisi View)
-- ===================================================

-- Buat view terlebih dahulu untuk memudahkan perhitungan
CREATE OR REPLACE VIEW RekapEmisi AS
SELECT 
    p.PenggunaID,
    (
        COALESCE(SUM(th.JarakHarian * fe_th.NilaiFaktor), 0) + 
...
LEFT JOIN ModaTransportasi mt_bt ON tbt.ModaID = mt_bt.ModaID
LEFT JOIN FaktorEmisi fe_bt ON mt_bt.fac_id = fe_bt.FaktorID
GROUP BY p.PenggunaID;

-- Query Top 10 Penyumbang Emisi
SELECT * FROM (
    SELECT 
        p.Nama,
        u.NamaUnit AS Unit,
        ...
        ROUND(
            ...
            NULLIF((SELECT SUM(TotalEmisiKG) FROM RekapEmisi), 0) * 100
...
-- ===================================================
-- 6. Tren Emisi (Bulanan)
-- ===================================================
WITH EmisiBulanan AS (
    SELECT 
        TO_CHAR(a.TanggalAktivitas, 'YYYY-MM') AS Bulan,
    ...
    LEFT JOIN ModaTransportasi mt_bt ON tbt.ModaID = mt_bt.ModaID
    LEFT JOIN FaktorEmisi fe_bt ON mt_bt.fac_id = fe_bt.FaktorID
    WHERE a.TanggalAktivitas BETWEEN DATE '2024-01-01' AND DATE '2024-06-30'
...
FROM EmisiBulanan A
LEFT JOIN EmisiBulanan B ON A.AngkaBulan = B.AngkaBulan + 1 
ORDER BY A.Bulan;
