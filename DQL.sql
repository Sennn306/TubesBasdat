...
SELECT 
    u_parent.NamaUnit AS Fakultas,
    COUNT(DISTINCT p.PenggunaID) AS JumlahAnggota,
    -- Penjumlahan standar
    ...
    AS TotalEmisi_KgCO2
FROM Unit u_parent
-- Join ke anak unit (Prodi/Lab)
JOIN Unit u_child ON u_parent.UnitID = u_child.UnitParentID
-- Join ke Pengguna
JOIN Pengguna p ON u_child.UnitID = p.UnitID
-- Join ke Aktivitas
LEFT JOIN Aktivitas a ON p.PenggunaID = a.PenggunaID 
-- Join Detail Tabel (Sama seperti query 1)
...
WHERE u_parent.JenisUnit = 'Akademik' -- Hanya ambil Fakultas
GROUP BY u_parent.UnitID, u_parent.NamaUnit
ORDER BY TotalEmisi_KgCO2 DESC;
